...
from ..services.patient_service import get_comprehensive_patient_data_for_recommendations, get_patient_comprehensive_data
...
def get_treatment_recommendations_api(patient_id: int):
    current_user_payload = get_current_user();
    clinician_id_from_token = current_user_payload.get('sub') if isinstance(current_user_payload, dict) else 1
    ...
    try:
        ...
        log_entry = PatientTreatmentRecommendationLog(
            patient_id=patient_id, input_features_json=engine.get_last_input_features(patient_id) or {},
            recommendations_json=[rec.dict() for rec in recommendations_resp_list],
            model_version_used=model_version, clinician_id=clinician_id_from_token
        )
        db.add(log_entry); db.commit()
        response_data = RecommendationResponse(
            patient_id=patient_id, recommendations=recommendations_resp_list,
            generation_timestamp=datetime.utcnow(), model_version=model_version
        )
        return jsonify(response_data.dict()), 200
    except Exception as e:
        db.rollback(); current_app.logger.error(f"Recommendations error patient {patient_id}: {e}", exc_info=True)
...
def analyze_combination_api(patient_id: int):
    ...
    try:
        ...
        current_app.logger.error(f"Combination analysis error patient {patient_id}: {e}", exc_info=True)
...
def get_clinical_summary_api(patient_id: int):
    ...
    try:
        ...
        response_data = ClinicalSummaryResponse(
            patient_id=patient_id, summary_timestamp=datetime.utcnow(),
            clinical_summary=summary_dict, model_used=model_type_arg
        )
        return jsonify(response_data.dict()), 200
    except ValueError as ve: return jsonify({"error": "Invalid model type for summarizer", "details": str(ve)}), 400
    except Exception as e:
        current_app.logger.error(f"Clinical summary error patient {patient_id}: {e}", exc_info=True)
